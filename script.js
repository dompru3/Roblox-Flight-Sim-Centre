...
// ── NAVIGATION ────────────────────────────────
// Exposed to window so inline onclick= attributes in HTML can call them
...
// ── ABOUT ─────────────────────────────────────
function renderAbout() { /* static content only */ }
      ...
      return `<div class="feed-item">
        <div class="feed-item-header">
          <div class="feed-item-meta">
            ...
            <div><div class="feed-username">${esc(s.username)}</div><div class="feed-time">${formatTime(s.timestamp)}</div></div>
          </div>
...
window.submitSuggestion = async function () {
  ...
  try {
    ...
    showAlert("sugAlert");
  } catch (e) { console.error(e); showAlert("sugError"); }
};

window.likeSug = async function (id) {
  try { await updateDoc(doc(db, "suggestions", id), { likes: increment(1) }); } catch (e) { console.error(e); }
};

window.deleteSug = async function (id) {
  if (!confirm("Remove this suggestion?")) return;
  try { await deleteDoc(doc(db, "suggestions", id)); } catch (e) { console.error(e); }
};
...
function renderComplaints() {
  ...
  feed.innerHTML = "<div class='loading-spinner'>Loading...</div>";
  const pc = { High: "#f08080", Medium: "#f7c65a", Low: "#5ec07a" };
  ...
  onSnapshot(q, snapshot => {
    ...
    feed.innerHTML = snapshot.docs.map(d => {
      ...
      return `<div class="feed-item" style="border-left:4px solid ${pc[c.priority] || "#5bafd6"}">
        <div class="feed-item-header">
          <div class="feed-item-meta">
            <div class="feed-avatar">&#x1F4E2;</div>
            <div><div class="feed-username">${esc(c.username)}</div><div class="feed-time">${formatTime(c.timestamp)}</div></div>
          </div>
...
window.submitComplaint = async function () {
  ...
  try {
    ...
    showAlert("cmpAlert");
  } catch (e) { console.error(e); showAlert("cmpError"); }
};
...
function renderPhotos() {
  ...
  onSnapshot(q, snapshot => {
    ...
    grid.innerHTML = snapshot.docs.map(d => {
      ...
      return `<div class="photo-card" onclick="openPhoto('${id}')">
...
window.confirmPhotoUpload = async function () {
  ...
  } catch (e) { console.error(e); showAlert("photoError"); }
};
...
// ── MORE / STATS ──────────────────────────────
function renderMore() { updateStats(); }
...
(function spawnClouds() {
  ...
  [
    ...
    el.appendChild(p1); el.appendChild(p2); sky.appendChild(el);
  });
})();
